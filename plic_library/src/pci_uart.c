...
static volatile uint32_t *pci_serial = (uint32_t *)(0x30000000 + 0x8000);	// found in gdb

static void
enable_io_access(volatile uint32_t *pci_dev)
{
	uint16_t *command = (uint16_t *)&pci_dev[1];
	*command = 0x1;		// enable IO
}

// offset from DTB_PCI_IO
static void
set_io_base(volatile uint32_t *pci_dev, uint64_t offset)
{
